...
FFmpegVideoDecoder::FFmpegVideoDecoder(bool testOnly)
    : m_Pkt(av_packet_alloc()),
      ...
      m_Pacer(nullptr),
      m_BwTracker(6, 200),  // [6 samples / 200ms window: faster adaptation vs original 10/250ms]
      m_FramesIn(0),
      m_FramesOut(0),
      m_LastFrameNumber(0),
      m_StreamFps(0),
        ...
        if (sfCount > 0) {
            ...
            for (int i = 0; i < sfCount && offset < length - 1; i++) {
                const char* stateStr = sfStats[i].active
                    ? u8"✔"   // ✔
                    : u8"✘";  // ✘
                // 優先顯示介面名稱（如「乙太網路」「Tailscale」），
                // 空名稱 fallback 到 type 名（Ethernet / VPN 等）
                const char* displayName = sfStats[i].interfaceName[0]
                    ? sfStats[i].interfaceName
                    : lcNetIfTypeName(sfStats[i].interfaceType);
                ret = snprintf(&output[offset], length - offset,
                    u8"%s %s: %.1fms / %.1fMbps / %.1f%%\n",
                    stateStr, displayName,
                    sfStats[i].rttMs,
