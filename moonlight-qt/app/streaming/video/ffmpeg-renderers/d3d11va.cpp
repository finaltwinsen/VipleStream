...
bool D3D11VARenderer::initialize(PDECODER_PARAMETERS params)
{
    ...
    // Use waitable swap chain with frame latency = 1 for minimal present queue depth.
    // This allows waitToRender() to block until the GPU is ready for a new frame,
    // reducing render-to-display latency compared to buffering excess frames.
    //
    // Buffer count: 1 front (max frame latency) + 1 back + 1 DWM DirectFlip + 1 AMD safety = 4
    //
    // NB: DXGI_SWAP_CHAIN_FLAG_FRAME_LATENCY_WAITABLE_OBJECT uses per-swapchain
    // latency control (IDXGISwapChain2::SetMaximumFrameLatency), which does NOT
    // cause SyncInterval 0 Present() calls to block like the per-device
    // IDXGIDevice1::SetMaximumFrameLatency() does.
    swapChainDesc.BufferCount = 1 + 1 + 1 + 1;
    ...
    // Set maximum frame latency to 1 for minimal present queue depth
    hr = m_SwapChain->SetMaximumFrameLatency(1);
    if (FAILED(hr)) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                     "IDXGISwapChain::SetMaximumFrameLatency(1) failed: %x",
                     hr);
        return false;
    }

    // Get the waitable object for GPU-ready signaling
    m_FrameLatencyWaitableObject = m_SwapChain->GetFrameLatencyWaitableObject();
    if (!m_FrameLatencyWaitableObject) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                     "IDXGISwapChain::GetFrameLatencyWaitableObject() returned null");
        return false;
    }
...
{
    // Wait for the GPU to be ready to accept a new frame.
    // The waitable swap chain object is signaled when the present queue
    // has room, ensuring we don't start rendering too early and pile up
    // frames in the GPU pipeline (which adds display latency).
    if (m_FrameLatencyWaitableObject) {
...
void D3D11VARenderer::renderFrame(AVFrame* frame)
{
    ...
    if (!m_FRUC && !m_GenericFRUC && !m_FRUCInitFailed
        ...
        //   - 720p 串流 + 90 fps 也想要 client-side interp
        //   - NCNN/Generic 兩個 backend 都不依賴 1080p（compute shader
        //     dispatch 維度跟 stream resolution 一致，dispatch 比 1080p
        //     小只會更快）
        //   - 真正會擋的是 GPU watchdog（4K iGPU TDR 已在 initFRUC 內部
        //     有 cap），不是 720p 太小
        // 直接呼叫 initFRUC()，讓 backend cascade 自己決定能不能跑。
        // 如果 initFRUC 失敗才顯示 overlay。
        if (!initFRUC()) {
