...
bool VulkanAV1Decoder::BeginPicture(VkParserPictureData* pnvpd)
{
    ...
    // VipleStream §J.3.e.2.i.8 Phase 3d.4f — upstream parser bug: setupSlotInfo
    // forgets to copy frame_type, while the parallel dpbSlotInfos[i] block
    // below DOES copy `m_pBuffers[i].frame_type`.  When subsequent inter
    // frames reference this slot, the decoder sees an inconsistent
    // frame_type record (its internal memo says X, our descriptor in
    // pStdReferenceInfo says Y) → alternating valid/corrupt outputs.
    av1->setupSlotInfo.frame_type = m_PicData.std_info.frame_type;
    // VipleStream §J.3.e.2.i.8 Phase 3d.4b — upstream parser bug: RefFrameSignBias
    // is OR-accumulated below without resetting, so bits set on earlier pictures
    // stick forever and the sign-bias bitmask grows monotonically.  This causes
    // incorrect inter-frame motion compensation and manifested as alternating
    // valid/garbage frames in the NV AV1 decoder.  Reset to 0 before the OR.
    av1->setupSlotInfo.RefFrameSignBias = 0;
    ...
    for (uint32_t i = 0; i < STD_VIDEO_AV1_NUM_REF_FRAMES; i++) {
        ...
        // VipleStream §J.3.e.2.i.8 Phase 3d.4b — same accumulation bug as setup slot above.
        av1->dpbSlotInfos[i].RefFrameSignBias = 0;
