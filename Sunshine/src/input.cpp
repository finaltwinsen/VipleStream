 ...
 */
// Must be defined before any header includes cmath/math.h
#define _USE_MATH_DEFINES
#include <cstdint>
#ifdef _MSC_VER
// MSVC doesn't provide __builtin_add_overflow; emulate using wider arithmetic.
// All usages here are on types narrower than int, so int is wide enough.
template<typename T>
static inline bool msvc_add_overflow(T a, T b, T *result) {
  int wide = (int) a + (int) b;
  *result = (T) wide;
  return wide != (int) *result;
}
#  define __builtin_add_overflow(a, b, res) msvc_add_overflow(a, b, res)
#endif
extern "C" {
...
#define ENABLE_LEFT_BUTTON_DELAY nullptr
  ...
  void passthrough_next_message(std::shared_ptr<input_t> input) {
    ...
    // batching process and allows the network thread to keep receiving inputs.
    {
    ...
    while (!local_queue.empty()) {
      ...
      std::vector<uint8_t> entry = std::move(local_queue.front());
      PNV_INPUT_HEADER payload = (PNV_INPUT_HEADER) entry.data();
      local_queue.pop_front();

      // Try to batch with remaining items on the local queue
      auto i = local_queue.begin();
      while (i != local_queue.end()) {
