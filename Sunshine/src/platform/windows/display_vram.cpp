...
// which would cause operator== overload conflicts on MSVC.
#include <d3d11.h>
  ...
  public:
    int convert(platf::img_t &img_base) {
      ...
      if (!img.blank) {
        ...
        // Wait for capture-side fence signal before using the texture
        {
          auto expected_val = img.fence_value.load(std::memory_order_acquire);
          if (expected_val > 0) {
            img_ctx.encoder_ctx4->Wait(img_ctx.encoder_fence.get(), expected_val);
          }
        }
        ...
        // No release needed — fence-based sync is unidirectional (capture signals, encoder waits)
...
#undef create_pixel_shader_helper
    ...
    struct encoder_img_ctx_t {
      ...
      fence_t encoder_fence;           // Shared fence opened on encoder device
      device_ctx4_t encoder_ctx4;      // D3D11.4 context for fence Wait
    ...
    int initialize_image_context(const img_d3d_t &img, encoder_img_ctx_t &img_ctx) {
      ...
      // Open the shared fence on the encoder device for GPU synchronization
      device5_t device5;
      status = device->QueryInterface(__uuidof(ID3D11Device5), (void **) &device5);
      if (FAILED(status)) {
        BOOST_LOG(error) << "Failed to query ID3D11Device5 for fence support [0x"sv << util::hex(status).to_string_view() << ']';
        return -1;
      }

      status = device5->OpenSharedFence(img.fence_shared_handle, __uuidof(ID3D11Fence), (void **) &img_ctx.encoder_fence);
      if (FAILED(status)) {
        BOOST_LOG(error) << "Failed to open shared fence on encoder device [0x"sv << util::hex(status).to_string_view() << ']';
        return -1;
      }

      // Get D3D11.4 device context for fence Wait operations
      status = device_ctx->QueryInterface(__uuidof(ID3D11DeviceContext4), (void **) &img_ctx.encoder_ctx4);
      if (FAILED(status)) {
        BOOST_LOG(error) << "Failed to query ID3D11DeviceContext4 [0x"sv << util::hex(status).to_string_view() << ']';
        return -1;
      }
  ...
  capture_e display_ddup_vram_t::snapshot(const pull_free_image_cb_t &pull_free_image_cb, std::shared_ptr<platf::img_t> &img_out, std::chrono::milliseconds timeout, bool cursor_visible) {
    ...
    auto get_locked_d3d_img = [&](std::shared_ptr<platf::img_t> &img, bool dummy = false) -> std::tuple<std::shared_ptr<img_d3d_t>, texture_fence_helper> {
      ...
      // Create fence helper that will signal the fence when it goes out of scope,
      // telling the encoder device that capture writes are complete.
      texture_fence_helper fence_helper(device_ctx4.get(), d3d_img.get());
      if (!fence_helper.arm()) {
        BOOST_LOG(error) << "Failed to arm fence helper for capture texture";
        return {nullptr, nullptr};
      }
      ...
      return {std::move(d3d_img), std::move(fence_helper)};
    };
  ...
  capture_e display_wgc_vram_t::snapshot(const pull_free_image_cb_t &pull_free_image_cb, std::shared_ptr<platf::img_t> &img_out, std::chrono::milliseconds timeout, bool cursor_visible) {
    ...
    if (complete_img(d3d_img.get(), false) == 0) {
      // Copy the captured frame and signal fence when done
      device_ctx->CopyResource(d3d_img->capture_texture.get(), src.get());
      if (device_ctx4 && d3d_img->capture_fence) {
        auto val = d3d_img->fence_value.fetch_add(1) + 1;
        device_ctx4->Signal(d3d_img->capture_fence.get(), val);
      }
    } else {
      return capture_e::error;
    }
  ...
  int display_vram_t::complete_img(platf::img_t *img_base, bool dummy) {
    ...
    if (img->fence_shared_handle) {
      CloseHandle(img->fence_shared_handle);
      img->fence_shared_handle = nullptr;
    }
    img->fence_value.store(0);
    img->data = nullptr;
    ...
    t.MiscFlags = D3D11_RESOURCE_MISC_SHARED_NTHANDLE | D3D11_RESOURCE_MISC_SHARED;
    ...
    // Create D3D11 fence for GPU-GPU synchronization (replaces keyed mutex)
    {
      device5_t device5;
      status = device->QueryInterface(__uuidof(ID3D11Device5), (void **) &device5);
      if (FAILED(status)) {
        BOOST_LOG(error) << "Failed to query ID3D11Device5 for fence creation [0x"sv << util::hex(status).to_string_view() << ']';
        return -1;
      }

      status = device5->CreateFence(0, D3D11_FENCE_FLAG_SHARED, __uuidof(ID3D11Fence), (void **) &img->capture_fence);
      if (FAILED(status)) {
        BOOST_LOG(error) << "Failed to create D3D11 fence [0x"sv << util::hex(status).to_string_view() << ']';
        return -1;
      }

      // Create shared handle for the fence so the encoder device can open it
      status = img->capture_fence->CreateSharedHandle(nullptr, GENERIC_ALL, nullptr, &img->fence_shared_handle);
      if (FAILED(status)) {
