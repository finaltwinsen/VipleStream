...
namespace platf {
  ...
  void adjust_thread_priority(thread_priority_e priority) {
    ...
    // Register with MMCSS to protect CPU time budget against interrupt storms and other high-priority tasks.
    // "Games" for critical threads (video capture/encode, stream control),
    // "Capture" for high-priority threads (packet send, video dispatch).
    if (mmcss_task) {
      DWORD task_index = 0;
      HANDLE handle = AvSetMmThreadCharacteristics(mmcss_task, &task_index);
      if (!handle) {
        BOOST_LOG(warning) << "AvSetMmThreadCharacteristics(\""sv << mmcss_task << "\") failed: "sv << GetLastError();
      }
    }
  }
