 ...
 */
// Must be defined before any header includes math.h/cmath to expose M_PI on MSVC
...
#define APPLY_CALIBRATION(val, bias, scale) (int32_t) (((float) (val) + (bias)) / (scale))
  ...
  static void ds4_update_motion(gamepad_context_t &gamepad, uint8_t motion_type, float x, float y, float z) {
    ...
    intX = std::clamp(intX, (int32_t) INT16_MIN, (int32_t) INT16_MAX);
    intY = std::clamp(intY, (int32_t) INT16_MIN, (int32_t) INT16_MAX);
    intZ = std::clamp(intZ, (int32_t) INT16_MIN, (int32_t) INT16_MAX);
