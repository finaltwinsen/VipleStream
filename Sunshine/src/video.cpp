...
// which would cause operator== overload conflicts in d3d11.h on MSVC.
#  include <d3d11.h>
extern "C" {
...
namespace video {
  ...
  encoder_t software {
    ...
    },
    {
      ...
      // aq-mode=1: re-enable Adaptive Quantization (disabled by 'superfast' preset),
      //            improves quality in flat regions (skies, walls) at no latency cost.
      {
        {"forced-idr"s, 1},
        {"x265-params"s, "info=0:keyint=-1:aq-mode=1:aq-strength=1.0"s},
    ...
    {
      // Common options
      // x264-params: re-enable Adaptive Quantization (disabled by 'superfast' preset)
      // and psy-rd for better perceptual quality without adding latency.
      {
        {"preset"s, &config::video.sw.sw_preset},
        {"tune"s, &config::video.sw.sw_tune},
