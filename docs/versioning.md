...
moonlight-android/app/build.gradle）。這些檔案的版號是由 `build-tools/version.ps1` 從
`version.json` 自動同步，手改會造成 drift。

## 2. 下游同步目標

`build-tools/version.ps1 Propagate-Version` 會把 `version.json` 同步到以下三個位置：
...
```powershell
pwsh build-tools\version.ps1 get      # -> 1.2.12
```
