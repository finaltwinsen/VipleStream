...
| ↳ Next attempts (post-D.a) | 真 latency gain 需要：(b) `VK_PRESENT_MODE_MAILBOX_KHR` 替代 FIFO（無 vsync block，最新 frame 贏），但會 tear；(c) probe `vkGetPhysicalDeviceQueueFamilyProperties.queueCount` — 如果 Adreno 620 universal family 有 ≥ 2 queue，建第二 queue 給 compute 真正 parallel；(d) 改 dual-present semantics：input < display rate 時才 dual present (60 FPS 在 60Hz 直接 single)，避免無謂 throttle。 | — |
