...
| **D** Async compute | 3–4 day | ME 移到專屬 compute queue，跟 present queue 平行；ME 跟前一幀 present 重疊 — **這才是 +17ms 延遲的真正解法**，也解 §I.C.5.b 觀察到的 dual present + waitIdle thermal regression；baseline 對比驗 latency 真的下降 | 同 C toggle，效能升級 | ⏸️ 等 C 完整 ship；C.5.b baseline 已 ship 為 §I.D 進入條件 |
