...
| **J.3.e.2.h** Generic FRUC port to Vulkan-native | v1.3.113 (h.a)：HLSL→GLSL 三條 shader 移植 + glslang 驗證。v1.3.114 (h.c)：Vulkan compute pipeline 結構（3 pipeline + 5 buffer + descriptor pool/sets）init + smoke dispatch 全通。**演算法層完全就位**（VIPLE_VK_FRUC_GENERIC=1 觸發 smoke）。h.d/e 阻塞於 §J.3.e.2.g —— 雙 present 接 libplacebo 會踩同一個 timeline bug | 🟡 演算法 ship、render 整合 blocked |
