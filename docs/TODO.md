...
| **J.3.e** FRUC 整合進 PlVkRenderer | 把 NCNN-Vulkan / GenericFRUC 從 D3D11VARenderer 移到 PlVkRenderer 的 renderFrame loop。AVVkFrame.img[0] 直接餵 ncnn::VkMat 不過 bridge | ⏳ 進行中 — §J.3.e.1.a-d (v1.3.65-71) + §J.3.e.2.a-e1b (v1.3.72-91) NV12→RGB compute / pl_vulkan_wrap render override / 3-phase pipeline 全部 ship。§J.3.e.2.e2 (RIFE forward 注入) 經 trace 確認 ncnn external VkDevice 的 VkMat input path 撞到 MSVC virtual-inheritance ABI 雷，**改走 Path B（CPU ncnn::Mat 中介，HOST_VISIBLE staging buffer）**。詳見 `docs/J.3.e.2_ncnn_renderframe_integration.md` |
| **J.3.e.2.g** Path B Phase A/C 同步重整 | v1.3.107 benchmark 發現 Path B pass-through 在 frame#2 systematic 掛在 Phase C fence wait timeout。v1.3.108-111 嘗試 cmd buf + fence 拆分、改走 single-cmd-buf 路徑、加 ENTRY trace 後**翻轉根因認定**：問題在 `runFrucOverridePass` 跟 libplacebo `pl_vulkan_hold_ex / release_ex` timeline 的互動，**§J.3.e.2.e1b 從第一版就有 latent sync bug**，跟 Phase A/C 拆分無關。詳見 docs/J.3.e.2_…md §J.3.e.2.g 段。短期應 disable `VIPLE_VK_FRUC_OUTPUT_OVERRIDE` 直到 g.A/g.B/g.C 三選一修好 | 🔴 阻塞所有 override 路徑（含 §J.3.e.2.h Vulkan-native FRUC）|
| **J.3.e.2.h** Generic FRUC port to Vulkan-native | v1.3.113 (h.a)：HLSL→GLSL 三條 shader 移植 + glslang 驗證。v1.3.114 (h.c)：Vulkan compute pipeline 結構（3 pipeline + 5 buffer + descriptor pool/sets）init + smoke dispatch 全通。**演算法層完全就位**（VIPLE_VK_FRUC_GENERIC=1 觸發 smoke）。h.d/e 撞上 libplacebo wrapped pl_tex 內部 sync 黑箱（§J.3.e.2.g）— 改走 §J.3.e.2.i 繞過 libplacebo | 🟡 演算法 ship、libplacebo 整合放棄 |
| **J.3.e.2.i** VkFrucRenderer (Android architecture port) | 既有 `moonlight-android/app/src/main/jni/moonlight-core/vk_backend.c` 4117 行 production-tested native Vulkan FRUC（自己 vkCreateSwapchainKHR + dual vkQueuePresentKHR + per-slot acquire/renderDone 雙 sem），完全不依賴 libplacebo。PC 端 port 70% 直接可用，30% 改 ffmpeg AVVkFrame import + NVIDIA present timing。設計 `docs/J.3.e.2.i_…md`，6 個 sub-phase（i.1-6），預期 ~1 週、~v1.3.117-124 | ⏳ 推進中 |
| **J.3.e.X** 手刻 RIFE Vulkan pipeline | 跳過 ncnn 整個依賴，自己用 ~10-15 個 raw VkPipeline 實作 RIFE forward；權重 mmap 進 VkBuffer，PipelineCache binary 開機建一次。預期 latency 從 Path B 的 ~12ms 壓到 ~3-5ms | 🟡 long-term — 觸發條件：Path B 量測後 PCIe staging cost 占 budget 30%+，或要推 1080p/4K，或要砍 ncnn DLL 依賴。**§J.3.e.2.g 修好之前不要動** |
