...
| **J.3.e.2.g** Path B Phase A/C 同步重整 | v1.3.107 benchmark 發現 Path B pass-through 在 frame#2 systematic 掛在 Phase C fence wait timeout。v1.3.108-111 嘗試 cmd buf + fence 拆分、改走 single-cmd-buf 路徑、加 ENTRY trace 後**翻轉根因認定**：問題在 `runFrucOverridePass` 跟 libplacebo `pl_vulkan_hold_ex / release_ex` timeline 的互動，**§J.3.e.2.e1b 從第一版就有 latent sync bug**，跟 Phase A/C 拆分無關。詳見 docs/J.3.e.2_…md §J.3.e.2.g 段。短期應 disable `VIPLE_VK_FRUC_OUTPUT_OVERRIDE` 直到 g.A/g.B/g.C 三選一修好 | 🔴 阻塞所有 override 路徑 |
