...
| **C.5** Quality preset wiring + GLES parity | 拉 `setQualityLevel(0/1/2)` 切換 ME pipeline；對 GLES baseline 跑 `scripts/benchmark/android/`，verify fps / latency 不退步。**進行中**：拆 C.5.a (wiring) → C.5.b (baseline 對比)。 | 中；perf 可能輸 GLES，要分析 |
