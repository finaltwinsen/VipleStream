...
| ↳ Known limitation | `vkGetRefreshCycleDurationGOOGLE` 在 Adreno 620 不會 update panel hot-switch 後的 refresh — workaround 是 trust `setFrameRate` hint。General fix: `AChoreographer_getFrameRate()` (API 33+) — return 當前 display rate；可加 dlsym fallback 自動偵測 panel rate 變化。目前 hint-based 對 Pixel 5 setup 工作，dlsym AChoreographer 留作 follow-up。 | — |
