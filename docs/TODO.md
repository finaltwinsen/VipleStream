...
| **Planned（下一步）** | **§P2** 壅塞感知 ABR | 把 `LiGetEstimatedRttInfo()`（現成、目前無人呼叫）接進 ABR：維護 min-RTT 基線 + EWMA，RTT 相對基線暴衝（bufferbloat）時**先於**丟包主動降碼，讓管線排空。與既有 QSTALE/FEC 回饋取「較保守者」，回升沿用 §ABR-RAMP-GATE 不另寫一套。再把 §FRZ-WATCHDOG「連續無效 reset + RTT 高」的情況改成觸發一次緊急降碼而非再要 IDR（治本；管線塞住時再要 IDR 只會加重壅塞）。**IDR 風暴防呆必做**（§K.9 前科）|
| **Planned（需先取證）** | **§P4** LAN 被判成 VPN → packetSize 1024 | `nvcomputer.cpp:572` 只要介面 MTU < 1500 就判 `RI_VPN` → `session.cpp:2259` 把 packetSize 壓到 1024 並標 `STREAM_CFG_REMOTE`。實測 6 場 LAN 直連（log 明寫 `Skipping hole punch: direct private address`）全部 `Interface MTU: 1280`——1280 是 Tailscale/WireGuard 通道的典型值，且兩端都跑 Tailscale。**先取證再改碼**：`scripts\diag\` 兩端 pktmon + `Get-NetRoute` 確認到 192.168.51.226 的實際出口介面。若真走通道 → 環境面修（程式不用改）；若是直連但 MTU 誤報 → 放寬啟發式成「MTU<1500 **且** 目標不在本網段」。⚠ 取證前不要動 packetSize 或加 PMTU 探測（§K.9-PMTUD-FIX 已在這塊踩過雷）|
...
| **Active (verify + polish)** | **§Q** MP-QUIC 多路徑網路聚合 | Phase 5 全 Sprint + §Q.path-recovery 完成 (v1.5.133)。Fix L–Q.5 (v1.5.192–202) 完成 failover/failback + 遠端單路徑韌性：Fix P IDR-QUIC-FIRST、Fix Q RTT 自適應 jitter timeout (1.5×RTT 夾 10-40ms)、Fix Q.2 §Q-STALE headroom gate（修 cwnd 健康卻丟光 video 的死亡螺旋）、Fix Q.3 FRUC throttle startup grace（修啟動 5s 模糊）、Fix Q.4 顯示器 graceful fallback + NVENC vbvInitialDelay（修啟動畫質）、Fix Q.5 lateAfterSkip/fecRedundantLate 診斷分離。**已驗證：** server FEC ✅、per-path queue ✅、0-RTT ticket ✅、1080p60 穩定 ✅、Audio QUIC ✅、Jitter buffer ✅、Ethernet failover/failback ✅ (v1.5.196)、遠端 WARP/Tailscale 單路徑 ✅ (v1.5.199 §Q-STALE 修後)、**Android moonlight-common-c 全量對齊 + Pixel 5 實機 MP-QUIC 串流驗證 ✅** (v1.5.201-202，lateAfterSkip=0/fecRedundantLate 實機確認)。**待做：** 壓力測試 1440p120。~~遠端高碼率評估 ABR~~ → **ABR 已於 v1.5.265 實作並上線**（§ABR-QSTALE-FEEDBACK / §ABR-RAMP-GATE / §ABR-FLOOR-DERP / §ABR-FLOOR-ESCAPE），本行原本的「再評估」已過時。**新缺口（2026-08-27 log 實證）：** ABR 目前只吃 FEC/QSTALE 的**丟包**回饋，完全沒用到 RTT——`LiGetEstimatedRttInfo()`（`ControlStream.c:2367`）existing 但無人呼叫。bufferbloat 的特徵正是「先塞爆延遲、後才丟包」，所以 ABR 全程失明：`VipleStream-1787406642.log` 場次 #3 實測控制通道 RTT 衝到 2827→4194ms 而丟包僅 0.12%，§FRZ-WATCHDOG 連 5 次 reset+IDR 全無效。見 §P2（壅塞感知 ABR）|
| **Active (shipped v1.5.258, 主線驗畢)** | **§Q-REGAIN** failover 後路徑自動回歸 | 2026-07-01 事故（有線串流間歇變糊、視訊困在 VPN 86 分鐘）root cause 修復：client §Q-STANDBY-REGAIN（rank 回歸 + flap 退避 + ICMP liveness）/ §Q-VPN-REDEMOTE / §Q-INACTIVE-REVIVE / §Q-REJECT-CACHE-TTL / §Q-REGAIN-BREAKER；server §Q-STICKY-ESCAPE（loss 視窗逃生門 + light IDR）/ §Q-ABR-FLOOR-ESCAPE / §Q-CNXQ-WARMUP。五輪 failover 回歸測試驗畢（`scripts\failover_regain_test.ps1`，不入 git；benchmark 黑板 round-2026-07-02-path-regain）。**剩（不阻塞）：** 乙太瞬斷外因（switch/ARP 層，兩端事件檔無 link 事件）；VPN 路徑持續 loss 機理（疑 Tailscale MTU 1280 黑洞）；ghost-slot 現象（picoquicDeleted slot 的 socket 仍收視訊，§K.10 歸因失真）；server 端 set_path_status 翻轉 client 本地 path_is_backup 而 tracked cache 不知情（Pass 3 既有缺口） |
...
| §SC-ID | §SC-HID Steam Controller 轉發：Round 1 四根因修復 | ✅ **Round 1 驗證通過（2026-09-02，v1.5.270 + driver 1.0.5.0）**：host Steam 辨識為 Steam Controller（USB）、真實屬性握手成功、按鍵正常；剩 Round 2 收尾 | **前情**：身分修復（2026-07-02，v1.5.259-261）讓 host Steam 認得 path/VID/PID，但握手從未成功——當時記的「ring 零事件」就是下面的 RC2b。使用者實體 SC 現改走 Puck 無線接收器（PID 0x1304），整條鏈卻是照 USB 直連時看到的 0x45 硬編。**本輪實測定案四根因**（計畫 `steam-controller-elegant-spindle.md` F1-F19）：**RC1 report id（client）** 經 Puck 的 gamepad state 是 `0x42`（~266 Hz、54 B、與 0x45 同 `TritonMTUNoQuat_t` 佈局），client 的 0x45 白名單把 100% 真實輸入丟掉，host 只收到 keepalive 零值；**RC2a feature 時序（client）** Puck 的回應是一次性暫存器、SET 後 13～21 ms 才出現、不讀就消失，舊碼 `SDL_Delay(20)`＋單次 GET 必失，且回應要比對 `resp[1]==type`（會撿到未請求的 `0x87` ack）；**RC2b feature handler（host driver）** UMDF `IOCTL_UMDF_HID_GET/SET_FEATURE` 的 buffer 慣例誤解（GET：reportId 在 input buffer[0]、report 寫進 output buffer；SET：report 在 input buffer、reportId 藏在 OutputBufferLength），舊碼把 64-byte buffer 硬轉 `HID_XFER_PACKET` 得 `reportBufferLen=0` → 兩個 handler 主體從未執行 → GET 回呼叫者原 buffer、SET 永不入 ring → host Steam `GetControllerInfo` 100% 失敗（每秒 `Read failure` → 10.8 s timeout → zombie → 每 ~12 s 重開）；**與 Puck 無關，USB 直連同樣壞**；**RC3 部署鏈** deploy 只複製 driver 檔從不跑 Install、`tryInstallDriver()` 只在開不到裝置時觸發一次、pnputil 對同 DriverVer 靜默跳過、`.cer` 沒進 zip → driver 改動永遠不在 host 生效。**Round 1 改動（WP0～3）**：driver feature handler 依 vhidmini2 UMDF2 慣例重寫 + GET(0x01) 閘控（SET 在途無回應回 STATUS_UNSUCCESSFUL＝user-mode err 31、與實體 Puck 相同；400 ms 逾時退回 LastResponse）+ descriptor 加 0x42 Input/Output + Feature 0x05 統計 report（0x03 維持純事件）+ WRITE(0x04) 全零 payload 忽略 + INF DriverVer 1.0.5.0；client 常數改 Valve 定義、0x42→0x45 正規化（`kNormalize42`，Plan B 關掉即原樣透傳）、feature 改 SET→每 1 ms 輪詢 GET 100 ms + type 比對、rx/feature 計數器與 `rx stats(5s/final)`／`Feature req … lat=`（全走 APPLICATION 類別）；server `VipleSCHidWrite` 放行 0x42、skip 回 2、`write stats(10s)` 加 drvSet/drvGet、Install 路徑先找 exe 同目錄；部署鏈 `Install-VipleSCHid.ps1` 版本比對自動 `-Reinstall` + 憑證前置檢查 + RESULT 帶 driverVersion/infVersion、`deploy_from_release.ps1` 以 host 現行版（https/curl、提權停服務、Copy-WithRetry）為底稿重做：停服務後跑 Install（獨立 powershell.exe + 180 s 逾時 kill；備份 `sc_hid_driver_prev\<既有 INF DriverVer>\` + `config\schid-deploy.log` + `-ForceDriverReinstall`；Start-Service 在 finally 保證拉起）、`build_sunshine.cmd` 5 檔缺一即 exit 1（含 `.cer`）、診斷腳本 `sc_hid_probe.ps1`（client）／`sc_hid_host_check.ps1`（host run-script；服務 Running 預設退 `-NoFeature`、讀 Feature 0x05 統計、verdict 拆 `GATE-OK-BUT-NO-SET-EVENT`）、setup_guide host 小節 + log_tags `[SC-HID]`。**commit 時注意：diag 腳本位於 `Sunshine/src/platform/windows/sc_hid_driver/diag/`**（`scripts/` 整個目錄是本機 gitignore，放那裡不會進 repo；host 黑板 `scripts` 也從這裡 `bb_write`）。driver 端定案：Feature 0x03 純事件、**Feature 0x05 = 統計**（SET/GET 次數、pending/ready、delivered、gated、zeroDropped、版本）、WRITE(0x04) 全零 payload 忽略、GET(0x01) 閘控逾時 400 ms、GET 事件節流 500 ms。**WP4 驗測結果（2026-09-02 22:23~22:28，串流 A＝本機 Steam 常駐，`bb_list benchmark` key `schid-round-1`）**：deploy 自動 `-Reinstall` → `bound=true driverVersion=1.0.5.0`；host_check `GATE-OK`（SET 入 ring、pending 期間 GET err 31、0x05 可讀）；client 6 分鐘 `rx stats(final)` total=88443 **id42=87688 fwd=87688 norm42=87688 sendErr=0**（全在 dev2=MI_02）、feature req 188 ok 186 stolen 0 lat avg 8/max 31 ms、暖機 0x83 成功；host `Session ended` **okS=86180 skippedS=0 failedS=1(err 1167，ForceReconnect 裝置重列舉窗口一次)** featureEvtsS=206 responsesS=180、driver 0x05 drvSet=177 drvGated=99 drvDelivered=172 drvZeroDrop=0；host Steam controller_ui.txt 22:23:05/22:23:12 兩次 `Controller 0 attributes: ProductID 4866 Serial FXA9961303086 Firmware Version 1784841029 Capabilities 4165bfff`（真實控制器屬性經 proxy 抵達）、22:23:12 `reserving XInput slot 0` 之後整場零 `Read failure`／零 zombie（部署前每 12 s 一次）；使用者：設定裡看到 Steam Controller（USB）、按鍵無異常。**G0～G5 全過**。**殘餘（不阻擋）**：①session 起始 host poll thread 先把 ring 內累積的 Steam SET（部署後 Steam 每秒查詢堆的 16 筆）轉給 client，早於 client `m_ScHid.start()`→ 10 行 `forwardFeatureRequest: no device open`（Steam 稍後重查即成功；Round 2 可讓 host 在 client passthrough 就緒前不 flush）；②2 筆 feature empty（type 0xC1 等控制器本身不回應的查詢）；③`write stats` 的 `drvVer` 位元組順序印成 5.1（source 已修為 1.5，待下次 server 建置）；④串流 B（本機 Steam 退出）未跑，A 已過故列為選配。**Round 2**：feature 回應改 RELIABLE（三份 common-c 同步）、Android `SteamControllerDriver.java` 對齊 0x42／輪詢、Steam Virtual Gamepad 抑制只在實機看到多一支 X360 才做、README highlights 隨 release。**Linux／Android 對齊現況不變**：udev rule + setup_guide 權限章節已 ship；Android JNI 橋 + USB claim 編譯過未實測（仍硬綁 0x45，Round 2 修） |
