...
| §SC-ID | §SC-HID 虛擬裝置身分修復 + 跨平台對齊 | ✅ **身分修復驗畢（2026-07-02，v1.5.259-261）；剩互動驗證** | **root cause（host 實測）**：虛擬裝置的子 HID 節點 HWID/path 不含 VID/PID（HIDClass 對 root-enumerated 裝置用「父節點名稱段」產生子節點 ID）→ Steam 靠 path 字串識別，看不出是 SC。**修法三件**：INF/Install HWID 改 `Viple\VID_28DE&PID_1302` + ROOT instance 名稱改 `VID_28DE&PID_1302`（→ 子節點 `HID\VID_28DE&PID_1302`、path `hid#vid_28de&pid_1302#…`）+ driver 補 GET_STRING（"Steam Controller"）+ attributes REV 0x0307。**host 實證**：Steam console_log 以正確 path 枚舉並辨識為 'Steam Controller' 28de:1302。配套：client 只轉發 0x45（puck 雜訊過濾）、server eager-poll（session 啟動即開 tunnel + feature 回應路徑自行 lazy-open，修「控制器睡眠時握手死鎖」）。**Linux 對齊**：程式碼本已跨平台編入，補 `moonlight-qt/scripts/linux/60-viplestream-steam-controller.rules` + setup_guide 權限章節 + client 端權限診斷 log。**Android 對齊**：JNI 橋（simplejni/callbacks/MoonBridge）+ `SteamControllerDriver.java`（USB claim + 0x45 轉發 + feature controlTransfer tunnel）+ UsbDriverService 整合 + controller_list gen-2 PID；APK 編譯過，未實測（需 prefConfig.usbDriver 開啟；BLE 0x1303 不支援）。**剩**：互動驗證（實體按鍵 → Steam 辨識 + 遊戲內輸入）；Steam 對既認得裝置何時重跑 GetControllerInfo 的時機仍未觀測到（ring 零事件），待互動場景確認 |
