...
| ↳ Future fix candidates | (b) `VK_PRESENT_MODE_MAILBOX_KHR` 替代 FIFO（dual mode 下移除 vsync block）— `30→60` / `45→90` 的 input < display 場景仍會被 (d) 觸發 dual，可能值得；(c) probe `vkGetPhysicalDeviceQueueFamilyProperties.queueCount` — Adreno 620 若 ≥ 2 queue，dual mode 下 compute 跑第二 queue 真正 parallel，dual mode latency 下降；目前 60 FPS in 60Hz 已完美 (d 解決)，等遇到 input < display case 再做。 | — |
