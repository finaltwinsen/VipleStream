...
| **Active (Phase B unblock, post-v1.4.111)** | **§B-NVOF Phase B prerequisite** m_NvOfFlowImage double-buffer | v1.4.110 證實 NVOF + ASYNC early-kickoff 跟 single-buffer flow image **Vulkan 同步 hazard 不可調和** (cmpCmd READ 跟 frame N 的 NVOF WRITE 跨 QF 並發). 真正修法: 兩張 m_NvOfFlowImage ping-pong (frame N NVOF 寫 image_A, frame N+1 cmpCmd 讀 image_A, frame N+1 NVOF 寫 image_B), 加 ping-pong handle tracking + 同步 sem. 需新 VkImage + memory (~3MB for 1080p R16G16) + ~150 行 changes + 跨 SW/HW path. **Phase B autotier 整合 (NVOF 當 NV best tier) block 在這條** — 修通才能 reapply early-kickoff + 拿回 5% async perf win + 才有意義把 NVOF 列 best tier. **Status (2026-05-18)**: v1.4.112-116 iteration 試 double-buffer + various reorder, 都遇到 bimodal driver implicit cross-QF wait (handoff 134us/15ms 隨機切, decode latency 累積到 200ms cycle). v1.4.115 baseline 1.98% drops 仍卡; v1.4.116 reorder 13.92% drops 更糟. 待 try VK_SHARING_MODE_CONCURRENT (v1.4.117 計劃) 或 buffer 輸出 (v1.4.118+ spike NVOF SDK) |
| ❌ **Dead-end** (NVOF SDK limitation) | **§B-NVOF Option D** NVOF buffer 輸出 取代 image 輸出 | 2026-05-18 spike NV OF SDK header (`nvOpticalFlowVulkan.h`) 確認: `NV_OF_REGISTER_RESOURCE_PARAMS_VK` struct **只有 `VkImage image` 欄位 + 唯一 register API `nvOFRegisterResourceVk`(image only)**. SDK 沒提供 buffer 變體 (e.g., `nvOFRegisterBufferVk`). NVOFA HW 內部用 image tiling layout 跑 (texture units / cache 對 2D image 友善), SDK 把硬體限制硬綁進 API. 要繞 = 改 SDK 或請 NV 加 API, 動不了. 紀錄此 dead-end 避免下次再 spike 同條死路 |
| ~~**Future (NVOF CPU latency)**~~ ✅ **SHIPPED v1.4.118 (§J.3.e.2.i.53) + v1.4.119 follow-ups (§J.3.e.2.i.54)** | **§B-NVOF Option B** NVOF dispatch 移到獨立 CPU thread | 2026-05-19 SHIPPED — v1.4.118 worker thread + bounded queue + condition_variable wake (renderer thread push, worker pop + SDK call + handle/value atomic). v1.4.119 (working tree) 加 m_NvOfWorkerInFlight in-flight counter + m_NvOfProfDroppedCount queue-full drop counter — 確保同幀內 partA copy / marker submit / push 三 site 決策一致, 避免 worker 在 site 之間結束導致 race. **Quit drain**: m_NvOfWorkerStop atomic + cv notify_all + join, queue 內 pending request 由 worker drain 後退出. **跟 GPU side fix 正交**: GPU-side §J.3.e.2.i.52 VK_SHARING_MODE_CONCURRENT (v1.4.117) 解 implicit wait, Option B 解 CPU SDK latency, 兩條都 ship 才完整解 Phase B 雙路 cmpCmd/NVOF concurrency. |
