...
| 優先級 | 條目 | 一句話 |
|---|---|---|
| **Active (next)** | **§J.3.e.X Path β.11** FRUC interp quality 微調 | β.11 hypothesis 1 baseline (EDGE_AWARE_MV_THRESHOLD 2.0→8.0) ship v1.4.11，待使用者實測決定：keep / 試 16 / 退 4 / 改 push constant runtime tunable |
| **Active (test pending)** | **§B Phase B restart** D3D11VA HEVC → Vulkan shared-image bridge | v1.4.167 ship B1-B8+B10 skeleton (D3D11VkBridge class + LUID match + SHARED_NTHANDLE export/import + ID3D11Fence ↔ Vulkan timeline VkSemaphore + CompositeMode enum + ablation env `VIPLE_VKFRUC_D3D11_HEVC_MODE` A1-A4)。動機：AMD Radeon 780M Vulkan driver 沒實作 `VK_KHR_video_decode_h265`，HEVC 全部走 SW decode 卡 ~50fps。**B9 (FRUC chain + present) 待 AMD 780M smoke 通過後接** — `set VIPLE_VKFRUC_D3D11_HEVC=1` 跑 HEVC stream 60s 不 device-lost 才可接下一階段；上次 NV 596.84 dead-end，這次 AMD 780M 是新驗證機會。NV laptop / 其他 GPU 預設關閉（env unset → false）行為不變 |
...
| **Active (follow-up)** | **§R2 PASSIVE FRUC** ratio controller | v1.4.169 §R2-η-2 ratio alignment gate + v1.4.170 §R2-θ 對齊 display Hz + 動態調 server fps via LiRequestFpsChange。**剩 follow-up:** (a) autotier T2→T0 跳 T1 (latency 一次破 budget 太多時直接 floor — 先觀察 alignment gate 生效後 latency 是否還會破到那麼大)；(b) `getRendererDisplayHz()` legacy fallback (renderer 沒實作 getter 回 0 走舊 `recv/server_fps` 行為 — 哪些 renderer path 該補) |
...
| **Active (K.2 smoke test)** | **§K.linux VAAPI→Vulkan bridge** (DMA-BUF interop) | K.1+K.2 ship：`vaapi_vk_bridge.{h,cpp}` + VkFrucRenderer VAAPI_VK composite mode + ffmpeg.cpp cascade。WSL Linux build PASS。待 Tailscale Vega 10 實機 smoke：stream H.264/HEVC 看 `[VIPLE-VAAPI-VK] frame#X import OK` log。K.3（接 FRUC chain）依賴 §B Phase B B9 完成 |
...
| **Active (β.10)** | **§J.3.e.X Path β.10** Linux / AMD / Intel 平台覆蓋 | Ubuntu noble compile + link PASS (v1.4.0 §K.X 對齊 + v1.4.169 GCC fix `unique_ptr<D3D11VkBridge>` incomplete type + X11 `None` macro 撞 enum)；Vega 10 Tailscale AppImage cold rc=0；待 native runtime smoke + AMD/Intel coopmat fallback 驗測 |
