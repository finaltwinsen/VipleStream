...
| **J.3.e.2.g** Path B Phase A/C 同步重整 | v1.3.107 benchmark 發現 Path B pass-through 在 frame#2 systematic 掛在 Phase C fence wait timeout（連 3 次重現）。原因是 Phase A 跟 Phase C 共用 `m_FrucOverrideCmdBuf` + `m_FrucOverrideFence`，跟 libplacebo swapchain present sync 排程衝突。修法：拆 cmd buf / fence，或全程改走 AVVkFrame timeline semaphore。詳見 docs/J.3.e.2_…md §J.3.e.2.f 段 | 🔴 阻塞 §J.3.e.X — 必須先修 |
