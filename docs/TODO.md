...
| **Active (Phase D.2.x ship + threshold tuned for 90Hz panel)** | **§I.D** Android Vulkan FRUC async compute | Adreno 620 / Pixel 5 verified: D.2.0 multi-queue acquisition + D.2.1 init cross-queue handoff + D.2.2/D.2.3 hot-path FRUC compute split (ycbcr+ME+median+warp+rotate on computeQueue, render passes on graphicsQueue, binary VkSemaphore between)。D.2.4 baseline + 真機 dual-mode trigger 都驗過：60fps Sunshine input + 90Hz Pixel 5 panel + FRUC enabled 預設進 dual mode 92.6%，2910 interp frames over 3000 native renders，p50 host frame 14.2ms (vsync-bound)，零 VK_ERROR。Threshold 從 1.05× 放寬到 1.40×（FIFO drop 不會 stutter，是當初保守設計留下的 false-floor）|
