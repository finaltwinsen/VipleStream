...
| **Future (NVOF CPU latency)** | **§B-NVOF Option B** NVOF dispatch 移到獨立 CPU thread | Render thread 目前 sync 呼叫 `nvOFExecuteVk` (SDK 1-15ms 變動 CPU time), 直接成 cmpCmd / 下幀 render 起跑延遲. 修法: 加 worker thread + lockfree queue, render thread push NVOF request 後立刻 return, worker pop + 呼叫 SDK + handle/timeline value 更新 (atomic). 預估 ~300 行. **跟 GPU side fix 正交**: VK_SHARING_MODE_CONCURRENT / buffer 輸出 解 GPU implicit wait, B 解 CPU SDK latency. **何時開**: GPU-side 修通 (handoff 穩定低) 但 cmpDur / total chain 仍因 SDK CPU latency 不穩才動 B. **複雜度**: thread sync + handle swap timing + failure 跨 thread + quit drain 都易出 bug, 需仔細設計 + race-condition 測試. Block 在更先做 GPU-side 修法上 |
