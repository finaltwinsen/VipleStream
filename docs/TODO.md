...
| **C.3** Compute pipeline scaffold | 在 VkBackend 加 4 個 `VkPipeline` (compute) + descriptor set layouts；分配 storage images（prev/curr RGBA、motionField、filteredMotionField、interpFrame）；**dispatch 但不 sample 結果到 swapchain**。**進行中**：拆 C.3.a (init/destroy only, no dispatch) → C.3.b (per-frame dispatch wired)。 | 中；storage image format / access flags |
