...
int quicConnect(const QUIC_CONNECT_PARAMS* params) {
    ...
    // Kick off the TLS handshake — generates the ClientHello into the
    // CRYPTO stream so the first Initial packet carries it.  Without
    // this call, picoquic_prepare_next_packet() only emits PING frames
    // and the handshake never starts (§K.4 root-cause).
    ret = picoquic_start_client_cnx(g_ctx.cnx);
    if (ret != 0) {
    ...
    Limelog("[VIPLE-MPQUIC] TLS ClientHello queued (handshake started)\n");
    ...
    // Bootstrap subflow: create a UDP socket bound to INADDR_ANY so the
    // IO thread can send/receive the initial TLS handshake.  Without this,
    // subflowCount == 0 when the IO thread starts and the Client Hello
    // packet has no socket to be sent from — causing a guaranteed 5s
    // handshake timeout.  This becomes subflow[0] / picoquic path 0.
    // Later, Connection.c adds real per-interface subflows (WiFi, 4G)
    // which become path 1, 2, ... via picoquic_probe_new_path().
    {
        struct sockaddr_storage anyAddr;
        SOCKADDR_LEN anyLen;
        SOCKET bootSock;
        QUIC_SUBFLOW* sf;
        ...
        if (serverAddr.ss_family == AF_INET) {
            struct sockaddr_in* sin4 = (struct sockaddr_in*)&anyAddr;
            sin4->sin_family = AF_INET;
            sin4->sin_port = 0;
            sin4->sin_addr.s_addr = INADDR_ANY;
            anyLen = sizeof(struct sockaddr_in);
        } else {
            struct sockaddr_in6* sin6 = (struct sockaddr_in6*)&anyAddr;
            sin6->sin6_family = AF_INET6;
            sin6->sin6_port = 0;
            sin6->sin6_addr = in6addr_any;
            anyLen = sizeof(struct sockaddr_in6);
        }

        bootSock = createSocket(anyAddr.ss_family, SOCK_DGRAM, IPPROTO_UDP, false);
        if (bootSock == INVALID_SOCKET) {
            Limelog("[VIPLE-MPQUIC] Failed to create bootstrap socket\n");
            picoquic_delete_cnx(g_ctx.cnx);
            g_ctx.cnx = NULL;
            picoquic_free(g_ctx.quic);
            g_ctx.quic = NULL;
        ...
        if (bind(bootSock, (const struct sockaddr*)&anyAddr, anyLen) != 0) {
            Limelog("[VIPLE-MPQUIC] Failed to bind bootstrap socket\n");
            closeSocket(bootSock);
            picoquic_delete_cnx(g_ctx.cnx);
            g_ctx.cnx = NULL;
            picoquic_free(g_ctx.quic);
            g_ctx.quic = NULL;
        ...
        sf->interfaceIndex = -1;  // bootstrap, not a real interface
        sf->sock = bootSock;
        memcpy(&sf->localAddr, &anyAddr, anyLen);
        sf->addrLen = anyLen;
        sf->active = true;
        sf->lastRecvTime = currentTime;
        sf->picoquicPathId = 0;
        g_ctx.subflowCount = 1;

        Limelog("[VIPLE-MPQUIC] Bootstrap subflow created (path 0)\n");
    }
    ...
    if (ret != 0) {
        ...
        closeSocket(g_ctx.subflows[0].sock);
        g_ctx.subflows[0].sock = INVALID_SOCKET;
