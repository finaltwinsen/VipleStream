    ...
    private boolean runPoll(ComputerDetails details, boolean newPc, int offlineCount) throws InterruptedException {
        ...
        try {
            if (!pollComputer(details)) {
                ...
                // VipleStream: If all direct addresses failed, try relay
                if (!details.state.equals(ComputerDetails.State.ONLINE)) {
                    PreferenceConfiguration prefs = PreferenceConfiguration.readPreferences(ComputerManagerService.this);
                    if (prefs.relayUrl != null && !prefs.relayUrl.isEmpty() && details.uuid != null) {
                        LimeLog.info("[VIPLE-NAT] Direct poll failed, trying relay for " + details.name);
                        RelayClient.LookupResult result = RelayClient.lookup(prefs.relayUrl, prefs.relayPsk, details.uuid, 5000);
                        if (result != null && result.success) {
                            // Server is on relay. Get serverinfo via HTTP proxy.
                            String serverInfoResp = RelayClient.httpProxy(prefs.relayUrl, prefs.relayPsk, details.uuid,
                                    "/serverinfo?uniqueid=0123456789ABCDEF", 8000);
                            if (serverInfoResp != null && !serverInfoResp.isEmpty()) {
                                // Parse serverinfo but preserve cached pair state
                                // (relay proxy can't present client TLS cert, so PairStatus is always 0)
                                PairingManager.PairState cachedPairState = details.pairState;
                                String cachedAppList = details.rawAppList;

                                try {
                                    NvHTTP tempHttp = new NvHTTP(details.activeAddress != null ? details.activeAddress :
                                            new ComputerDetails.AddressTuple("127.0.0.1", 47989),
                                            details.httpsPort, idManager.getUniqueId(), details.serverCert,
                                            PlatformBinding.getCryptoProvider(ComputerManagerService.this));
                                    ComputerDetails relayDetails = tempHttp.getComputerDetails(serverInfoResp);

                                    if (relayDetails != null && details.uuid.equals(relayDetails.uuid)) {
                                        // Update fields but preserve security state
                                        details.name = relayDetails.name;
                                        details.state = ComputerDetails.State.ONLINE;
                                        details.serverCert = details.serverCert; // keep cached

                                        // Preserve pair state: relay shows unpaired, but we trust cache
                                        if (cachedPairState == PairingManager.PairState.PAIRED) {
                                        ...
                                        // Store STUN info from relay lookup
                                        if (result.stunIp != null && !result.stunIp.isEmpty()) {
                                            details.stunAddress = result.stunIp;
                                        }
                                        if (relayDetails.stunAddress != null) {
                                            details.stunAddress = relayDetails.stunAddress;
                                        }
                                        details.stunNatType = result.natType;

                                        LimeLog.info("[VIPLE-NAT] " + details.name + " ONLINE via relay");
                                    }
                                } catch (Exception e) {
                                    // Parsing failed but server is reachable via relay
                                    details.state = ComputerDetails.State.ONLINE;
                                    if (cachedPairState == PairingManager.PairState.PAIRED) {
                                        details.pairState = PairingManager.PairState.PAIRED;
                                    }
                                    LimeLog.info("[VIPLE-NAT] " + details.name + " online via relay (parse failed: " + e.getMessage() + ")");
                                }
                            }
                        }
                    }
