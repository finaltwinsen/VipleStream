...
| Version | Changes |
|---|---|
| **1.5.108** | **§K.10–§K.16 MP-QUIC 穩定性 + Linux 完整建置 + §SINGLE-INST 單一實例守門。** §K.13 QuicTransport / Sunshine `quic_server::drainPendingToQuic` 三段式 UAF crash fix 對齊 Windows / Linux / Android 三端 — server `drainPendingToQuic` 入口加 `picoquic_get_cnx_state >= disconnecting` bail（client 突然斷線觸發 picoquic 內部 mass path-deletion 時，drain thread 跟 ioLoop cleanup race 會命中已 freed `picoquic_path_t`，v1.5.106 client dmp 已實證在 `sender.c:826 picoquic_predict_packet_header_length` NULL deref）；§K.10 IDR shard drop fix（v1.5.74 aggressive flush 在 slow-start cwnd ~14KB 砍掉只能送 ~10 shards 的 IDR frame 造成黑畫面，改純診斷不 drop）；§K.11/§K.12 BBR + datagram queue 診斷 telemetry；§K.16 BBRMinPipeCwnd 128 地板。§SINGLE-INST main.cpp Windows `CreateMutexW` + Linux `flock` 單實例守門，CLI 模式 attach 父 console 印 stderr / GUI 模式彈 MessageBox（v1.5.39 實測踩到三個實例同時連線搶 QUIC port + server session + log 混在一起）。`NvNetworkInterface` + `getNetworkInterfaceList(serverInfo)` 解析 `/serverinfo` 廣播的多介面（Tailscale VPN + Ethernet + WiFi）給 multi-path scheduler 選路。Linux 建置管線正式啟用 — `build-appimage.sh` 加 `DEFINES+=VIPLE_MPQUIC`，`modelfetcher.cpp/.h` 從 `win32:!winrt` 搬到 `libplacebo` block（純 Qt API 跨平台、`vkfruc.cpp` `ensureRifeModelDir()` Linux 也要用）；WSL ext4 sync 腳本補 moonlight-common-c QUIC source + picoquic headers。Linux 雙向 3 分鐘穩定性驗測通過：Linux Client→Windows Server NVENC 59.99 fps；Windows Client→Linux Server VAAPI+KMS 60.02 fps / decodeMean 0.29ms / networkDropped=0。Release 完整三件 + Linux 兩件：Client zip / Server zip / Android apk / Client AppImage / Server .deb 全 1.5.108 對齊。 |
