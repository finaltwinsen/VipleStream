...
| Version | Changes |
|---|---|
| **1.3.340 (β.8 beta)** | §J.3.e.X Path β — Native RIFE Vulkan 補幀新路線：在 Vulkan compute pipeline 跑 RIFE-v4.25-lite ML 抽出 motion flow + blend mask，再用自家 warp+blend shader 在 native 1080p 合成 interp frame.  Quality 大躍進 vs block-match (verify_dump_interp score 0.95 ≈ perfect midpoint vs block-match 0% effective interp).  Latency ~14ms 整 chain at 256×128 infer dim on RTX 3060 (60fps DUAL OK), ~10ms at 128×128 (75fps server / 154fps display). UI toggle 在 Settings → Frame Interpolation → 「Native RIFE 補幀 (β beta)」, 配 InferDim 下拉 (128/256/384/512). env var dev escape: `VIPLE_VKFRUC_NATIVE_RIFE=1` / `VIPLE_VKFRUC_RIFE_INFER_DIM=N`. **已知問題**：RTX 3060 + NV driver 596.144 連續 30-60s 後 GPU device-lost crash，root cause 待 Nsight Graphics 分析 — 故 default OFF + UI 標 (β beta) 警告. 詳見 `docs/J.3.e.X_path_b.md`. 9 個 commit shipped: 67a7c89 → 7d37f7a (β.5.1) + 此 commit (β.8 UI/docs). β.6 descSet cache attempt 試了 reverted (latency 退化 + 沒解 crash) |
